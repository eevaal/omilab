from sqlalchemy import select, Sequence, or_, func
from sqlalchemy.ext.asyncio import AsyncSession

import schemas.lectures
...
async def search_lectures(db: AsyncSession, query: str) -> Sequence[Lecture]:
    ...
    stmt = select(Lecture).where(or_(Lecture.title.ilike(search_term), Lecture.subject.ilike(search_term)))
...
async def get_lecture_by_id(db: AsyncSession, lecture_id: int):
    ...
    result = await db.execute(query)
    return result.scalar_one_or_none() # Вернет объект или None, если не нашел
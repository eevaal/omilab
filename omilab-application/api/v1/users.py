...
async def toggle_follow(
    ...
    if is_following:
        ...
        stmt = insert(subscriptions).values(follower_id=current_user.id, followed_id=target_user_id)
        action = "followed"
    ...
    count_query = (
        ...
        .where(subscriptions.c.followed_id == target_user_id)
    )
    new_followers_count = await db.scalar(count_query)

    return {"status": "ok", "action": action, "new_followers_count": new_followers_count}
